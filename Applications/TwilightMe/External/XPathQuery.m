...
NSArray *PerformXPathQuery(xmlDocPtr doc, NSString *query, NSString *nsPrefix, NSString *nsURI)
{
	...
	/* Add the optional namespace */
	if(nsPrefix && nsURI)
	{
		xmlXPathRegisterNs(xpathCtx, (xmlChar *)[nsPrefix cStringUsingEncoding:NSUTF8StringEncoding], (xmlChar *)[nsURI cStringUsingEncoding:NSUTF8StringEncoding]);
	}
...
NSArray *PerformHTMLXPathQuery(NSData *document, NSString *query, NSString *nsPrefix, NSString *nsURI)
{
    xmlDocPtr doc;
	...
	NSArray *result = PerformXPathQuery(doc, query, nsPrefix, nsURI);
    xmlFreeDoc(doc); 
	
	return result;
}

NSArray *PerformXMLXPathQuery(NSData *document, NSString *query, NSString *nsPrefix, NSString *nsURI)
{
    xmlDocPtr doc;
