...
{
	ARImageWrapper *wrapper = [[ARImageWrapper alloc] init];
	wrapper.image=theImage;
	wrapper.ownsImage=true;
	return [wrapper autorelease];
}

+ (ARImageWrapper *) imageWithCPPImage:(ARImage *) theImage ownsImage:(bool) ownsTheImage;
{
	ARImageWrapper *wrapper = [[ARImageWrapper alloc] init];
	wrapper.image=theImage;
...
ARImage::ARImage(ARImageWrapper *other, int x1, int y1, int x2, int y2) {
	m_width=x2-x1;
	m_height=y2-y1;
	m_imageData=(uint8_t *) malloc(m_width*m_height);
	initYptrs();
	ARImage *otherImage=other.image;
	for(int y=y1; y<y2; y++) {
		for(int x=x1; x<x2; x++) {
...
ARImage::ARImage(int width, int height) {
...
ARImage::ARImage(uint8_t *imageData, int width, int height, bool ownsData) {
...
ARImage::ARImage(UIImage *srcImage, int width, int height,  CGInterpolationQuality interpolation, bool imageIsRotatedBy90degrees) {
...
ARImageWrapper *ARImage::createImage(ARImageWrapper *other, int x1, int y1, int x2, int y2)
{
	return [ARImageWrapper imageWithCPPImage:new ARImage(other, x1, y1, x2, y2)];
}
// create an empty image of the required width and height
ARImageWrapper *ARImage::createImage(int width, int height) {
	return [ARImageWrapper imageWithCPPImage:new ARImage(width, height)];
}
// create an image from data
ARImageWrapper *ARImage::createImage(uint8_t *imageData, int width, int height, bool ownsData) {
	return [ARImageWrapper imageWithCPPImage:new ARImage(imageData, width, height, ownsData)];
}
// take a source UIImage and convert it to greyscale
ARImageWrapper *ARImage::createImage(UIImage *srcImage, int width, int height, bool imageIsRotatedBy90degrees) {
	return [ARImageWrapper imageWithCPPImage:new ARImage(srcImage, width, height, kCGInterpolationHigh, imageIsRotatedBy90degrees)];
}

void ARImage::extractConnectedRegion(int x, int y, std::vector<ARImagePoint> *points) {
	(*points).push_back(ARImagePoint(x,y));
	(*this)[y][x]=0;
	int left, right;
	left=x-1;
	right=x+1;
	while(left>=0 && (*this)[y][left]!=0) {
		(*this)[y][left]=0;
		(*points).push_back(ARImagePoint(left,y));		
		left--;
	}
	while(right<m_width && (*this)[y][right]!=0) {
		(*this)[y][right]=0;
		(*points).push_back(ARImagePoint(right,y));		
		right++;
	}
...
inline int findThresholdAtPosition(int startx, int starty, int size, ARImage* src) {
	int total=0;
...
/*
ARImageWrapper* ARImage::autoLocalThreshold() {
	const int local_size=10;
	// now produce the thresholded image
	ARImage *result=new ARImage(m_width, m_height);
	// process the image
	int threshold=0;
	...
	return [ARImageWrapper imageWithCPPImage:result];
}
*/
...
ARImageWrapper* ARImage::autoLocalThreshold() {
	...
	return ARImage::createImage(result, m_width, m_height, true);
}

ARImageWrapper *ARImage::autoThreshold() {
	int total=0;
	int count=0;
	for(int y=0; y<m_height; y++) {
		for(int x=0; x<m_width; x++) {
...
thigh 0.60-0.90
*/
ARImageWrapper *ARImage::cannyEdgeExtract(float tlow, float thigh) {
	int gx[3][3]={ 
		{ -1, 0, 1 },
		{ -2, 0, 2 },
		{ -1, 0, 1 }};
	...
	ARImage *result=new ARImage(edge, resultWidth, resultHeight, true);
	return [ARImageWrapper imageWithCPPImage:result];	
}

// rotate by 90, 180, 270, 360
ARImageWrapper *ARImage::rotate(int angle) {
	ARImage* result;
	switch(angle) {
		case 90:
		case 270:
			result=new ARImage(m_height, m_width);
			break;
		case 180:
			result=new ARImage(m_width, m_height);
			break;
	}
	for(int y=0; y< m_height; y++) {
		for(int x=0; x<m_width; x++) {
...
ARImageWrapper *ARImage::gaussianBlur() {
	int blur[5][5]={ 
		{ 1, 4, 7, 4, 1 },
		{ 4,16,26,16, 4 },
		{ 7,26,41,26, 7 },
	...
	ARImage *result=new ARImage(m_width-5, m_height-5);
...
void ARImage::HistogramEqualisation() {
...
ARImageWrapper *ARImage::resize(int newX, int newY) {
	ARImage *result=new ARImage(newX, newY);
...
void ARImage::findLargestStructure(std::vector<ARImagePoint> *maxPoints) {
	// process the image
	std::vector<ARImagePoint> points;
...
int findHeightAtX(ARImage *img, int x) {	
